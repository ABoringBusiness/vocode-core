...
from vocode.streaming.models.agent import EnhancedGroqAgentConfig
...
class EnhancedGroqAgent(RespondAgent[EnhancedGroqAgentConfig]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize Groq client with appropriate configuration
        api_key = agent_config.groq_api_key or os.environ.get("GROQ_API_KEY")
        if not api_key:
            raise ValueError("Groq API key must be provided in agent_config or as an environment variable")
        ...
        self.groq_client = AsyncGroq(**client_kwargs)
    ...
    def get_chat_parameters(self, messages: Optional[List] = None, use_functions: bool = True):
        ...
        if self.agent_config.stop_sequences:
            parameters["stop"] = self.agent_config.stop_sequences
    ...
    async def _create_groq_stream(self, chat_parameters: Dict[str, Any]) -> AsyncGenerator:
        try:
            stream = await self.groq_client.chat.completions.create(**chat_parameters, stream=True)
