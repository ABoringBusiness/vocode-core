...
class EnhancedOpenAIAgent(RespondAgent[EnhancedOpenAIAgentConfig]):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        api_key = agent_config.openai_api_key or os.environ.get("OPENAI_API_KEY")
        if not api_key:
            raise ValueError("OpenAI API key must be provided in agent_config or as an environment variable")
        ...
        if agent_config.organization_id:
            client_kwargs["organization"] = agent_config.organization_id
    ...
    def get_chat_parameters(self, messages: Optional[List] = None, use_functions: bool = True):
        ...
        messages = messages or format_openai_chat_messages_from_transcript(
            ...
            functions=self.functions if use_functions else None,
        ...
        if self.agent_config.response_format:
            parameters["response_format"] = self.agent_config.response_format
            
        # Add seed if specified for reproducibility
        if self.agent_config.seed is not None:
            parameters["seed"] = self.agent_config.seed
        ...
        if self.agent_config.frequency_penalty is not None:
            parameters["frequency_penalty"] = self.agent_config.frequency_penalty
            
        # Add presence_penalty if specified
        if self.agent_config.presence_penalty is not None:
            parameters["presence_penalty"] = self.agent_config.presence_penalty
            
        # Add logit_bias if specified
        if self.agent_config.logit_bias:
            parameters["logit_bias"] = self.agent_config.logit_bias

        if use_functions and self.functions:
            parameters["functions"] = self.functions
            if self.agent_config.function_call:
                parameters["function_call"] = self.agent_config.function_call
    ...
    async def generate_response(
        ...
        if not bot_was_in_medias_res and self.should_backchannel(human_input):
        ...
        if backchannel is not None:
            ...
            # between sending the response and generating OpenAI's response
            chat_parameters["messages"].append({"role": "assistant", "content": backchannel.text})
